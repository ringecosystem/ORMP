...
ormp_out=$(cast create2 -i $ormp_initcode -d $create2 --starts-with "00000000000000" | grep -E '(Address:|Salt:)')
ormp_addr=$(echo $ormp_out | awk '{print $2}' )
ormp_salt=$(cast --to-uint256 $(echo $ormp_out | awk '{print $4}' ))
echo -e "ORMP: \n Address: $ormp_addr \n Salt:    $ormp_salt"

oracle_args=$(ethabi encode params -v address ${dao:2} -v address ${ormp_addr:2})
oracle_initcode=$oracle_abi$oracle_args
oracle_out=$(cast create2 -i $oracle_initcode -d $create2 --starts-with "00000000000000" | grep -E '(Address:|Salt:)')
...
relayer_out=$(cast create2 -i $relayer_initcode -d $create2 --starts-with "00000000000000" | grep -E '(Address:|Salt:)')
